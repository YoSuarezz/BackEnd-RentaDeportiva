...
import co.edu.uco.unidaddeportivaelbernabeu.business.fachade.concrete.tarifas.ConsultarTarifasEstandarFachadaImpl;
...
public class TarifaEstandarController {
    ...
    @GetMapping
    public ResponseEntity<TarifaEstandarResponse> consultar() {
        var httpStatusCode = HttpStatus.ACCEPTED;
        var tarifaEstandarResponse = TarifaEstandarResponse.build();

        try {
            var facade = new ConsultarTarifasEstandarFachadaImpl();
            tarifaEstandarResponse.setDatos(facade.ejecutar(TarifaEstandarDTO.build()));
            tarifaEstandarResponse.getMensajes().add(MessageCatalogStrategy.getContenidoMensaje(CodigoMensaje.M00057));
        } catch (final UnidadDeportivaElBernabeuException excepcion) {
            httpStatusCode = HttpStatus.BAD_REQUEST;
            tarifaEstandarResponse.getMensajes().add(excepcion.getMensajeUsuario());
            excepcion.printStackTrace();
